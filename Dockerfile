# nexus-gateway — 容器化打包
# 构建: docker build -t nexus-gateway:v3.10 .
# 运行: docker run -d -p 8646:8646 nexus-gateway:v3.10
#
# 支持三种模式:
#   API 服务模式（默认）: python3 gateway.py
#   CLI 模式:           docker run --rm nexus-gateway:v3.10 python3 gateway.py check-deps
#   基准评分:           docker run --rm -v ./data:/app/data nexus-gateway:v3.10 python3 gateway.py benchmark --all
#
